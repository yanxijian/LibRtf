	...
	case '\\':
		...
		case '\'':
			...
			// append( & part->word, "\\\'");
			{
				...
				if( a >= 'a' && a <= 'f' ) _a = a - 'a' + 0x0A;
				else if( a >= 'A' && a <= 'F' ) _a = a - 'A' + 0x0A;
				else if( a >= '0' && a <= '9' ) _a = a - '0';
				else _a = 0;

				if( b >= 'a' && b <= 'f' ) _b = b - 'a' + 0x0A;
				else if( b >= 'A' && b <= 'F' ) _b = b - 'A' + 0x0A;
				else if( b >= '0' && b <= '9' ) _b = b - '0';
				else _b = 0;

				append_char( part, _a * 0x10 + _b );
