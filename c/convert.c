	...
	while (part)
	{
		...
		else if ( part->type == PART_TYPE_BLOCK_END )
		{
			if( fontblock_level == 1 )
			{
				...
				proc->iface->register_font(proc->iface, fontid, fontname.content, charset );
...
static int cmd_u (internal_processor *f, rtf_part *w) {
	...
	// FIXME: cast get_param() to (signed short)
	if( f->iface->print_unicode(f->iface, get_param(w) ) )
	{
		/***
			This keyword represents a single Unicode character that has no
			equivalent ANSI representation based on the current ANSI code
			page. N represents the Unicode character value expressed as a
			decimal number.This keyword is followed immediately by 
			equivalent character(s) in ANSI representation. In this way,
			old readers will ignore the \uN keyword and pick up the ANSI
			representation properly. When this keyword is encountered,
			the reader should ignore the next N characters, where N
			corresponds to the last \ucN value encountered.As with all RTF
			keywords, a keyword-terminating space may be present (before
			the ANSI characters) that is not counted in the characters to
			skip. While this is not likely to occur (or recommended), a
			\bin keyword, its argument, and the binary data that follows
			are considered one character for skipping purposes.
			If an RTF scope delimiter character (that is, an opening or
			closing brace) is encountered while scanning skippable data,
			the skippable data is considered to be ended before the
			delimiter. This makes it possible for a reader to perform
			some rudimentary error recovery. To include an RTF delimiter
			in skippable data, it must be represented using the
			appropriate control symbol (that is, escaped with a backslash,)
			as in plain text. Any RTF control word or symbol is considered
			a single character for the purposes of counting skippable
			characters.An RTF writer, when it encounters a Unicode
			character with no corresponding ANSI character, should output
			\uN followed by the best ANSI representation it can manage.
			Also, if the Unicode character translates into an ANSI
			character stream with a count of bytes differing from the
			current Unicode Character Byte Count, it should emit the \ucN
			keyword prior to the \uN keyword to notify the reader of the
			change.RTF control words generally accept signed 16-bit numbers
			as arguments. For this reason, Unicode values greater than
			32767 must be expressed as negative numbers.
		***/
		
		// FIXME:
		// We need to get latest \uc<n> value and then  skip <n> next characters.

		rtf_part * part = get_next_part( f->src );
		if( part->type == PART_TYPE_TEXT )
			return FALSE;

		// There was no ANSI character after \u 
		push_back_current_part( f->src ); 
	}
	return FALSE;
}
#endif
...
process_block (internal_processor * proc, rtf_part * part )
{
	...
	while ( part ) 
	{
		if ( part->type == PART_TYPE_TEXT || part->type == PART_TYPE_HEX_CHAR) 
		{
				...
				/*----Paragraph alignment----------------------------------------------------*/
		{
			...
			else if (! strcmp( "*", s) && commands == 1 ) // FIXME: && no_text && no_others
				command_commented = TRUE;
			else if (!strcmp ("pard", s))
			{
				...
				if( proc->coming_pars_that_are_tabular )
					--proc->coming_pars_that_are_tabular;
			}
			else if (!strcmp ("pict", s))
			{
				within_picture = TRUE;
				cmd_pict( proc, part );
			}
